...
using Auditor.Server.Resources;
...
namespace Auditor.Server.Helpers
{
    public class SpanishIdentityErrorDescriber : IdentityErrorDescriber
    {
        ...
        {
            var type = typeof(IdentityErrorMessagesResource);
            var assemblyName = new AssemblyName(type.GetTypeInfo().Assembly.FullName);
            _localizer = factory.Create("IdentityErrorMessagesResource", assemblyName.Name);
        }
        ...
        public override IdentityError UserLockoutNotEnabled()
        {
            return new IdentityError
            {
                Code = nameof(UserLockoutNotEnabled), Description = _localizer["Lockout is not enabled for this user."]
            };
        }
        ...
        public override IdentityError UserNotInRole(string role)
        {
            return new IdentityError
            {
                Code = nameof(UserNotInRole), Description = string.Format(_localizer["User is not in role {0}."], role)
            };
        }
